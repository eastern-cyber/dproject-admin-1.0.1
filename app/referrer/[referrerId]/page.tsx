...
export default async function ReferrerDetails({ params }: { params: { referrerId: string } }) {
  ...
  const navigateToMintingPage = () => {
    const data = {
      var1: params.referrerId || "N/A",
      var2: referrerData?.email || "N/A",
      var3: referrerData?.name || "N/A",
      var4: referrerData?.tokenId || "N/A",
    };
  ...
  return (
    <main className="p-4 pb-10 min-h-[100vh] flex flex-col items-center">
      <div className="flex flex-col items-center justify-center p-10 m-5 border border-gray-800 rounded-lg">
        ...
        <h1 className="p-4 md:text-2xl text-2xl font-semibold md:font-bold tracking-tighter">สมัครใช้งาน</h1>
        <WalletConnect />
        <div className="flex flex-col items-center justify-center p-2 m-2">
          ...
          {referrerData ? (
            <div className="mt-4 text-center gap-6 bg-gray-900 p-4 border border-1 border-gray-400">
              <p className="text-lg text-gray-300">
                <b>เลขกระเป๋าผู้แนะนำ:</b> {params.referrerId ? `${params.referrerId.slice(0, 6)}...${params.referrerId.slice(-4)}` : "ไม่พบกระเป๋า"}
              </p>
              <p className="text-lg text-gray-300">
          ...
          <div className="items-centerflex border border-gray-400 bg-[#2b2b59] p-2.5 mt-5 w-full">
            <p className="text-[18px] break-all">
              <center>{params.referrerId ? `${params.referrerId}` : "ไม่พบกระเป๋า"}</center>
            </p>
          </div>
        </div>
        <div className="flex flex-col items-center mb-6">
          <button
            onClick={navigateToMintingPage}
