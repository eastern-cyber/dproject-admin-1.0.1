...
export default function CheckUser() {
  ...
  const handleDownload = () => {
    ...
    // Create a Blob from the users data
    ...
    link.href = url;
    link.download = "download-dproject-users.json";
      ...
      >
        ...
        {matchingUser ? (
          <table className="table-auto border-collapse border border-gray-500 mt-4 w-full">
            <tbody>
              ...
              <tr>
                <td className="border border-gray-400 px-4 py-2">วันลงทะเบียนผู้ใช้</td>
                <td className="border border-gray-400 px-4 py-2">
                  {matchingUser.userCreated
                    ? new Date(matchingUser.userCreated).toLocaleDateString("en-GB") // 'en-GB' is for dd/mm/yyyy format
                    : "N/A"}
                </td>
              </tr>
              <tr>
        ...
        <div className="flex flex-col mt-8 justify-center items-center w-full">
          <button
            ...
            className="border border-blue-500 px-4 py-3 rounded-lg hover:bg-blue-600 transition-colors"
          >
            <p className="text-center text-[19px]">ดาวน์โหลดข้อมูลผู้ใช้ (JSON)</p>
          </button>
          <Link
            className="border border-zinc-500 px-4 py-3 rounded-lg hover:bg-red-600 transition-colors hover:border-zinc-800 mt-4"
            href="/"
          >
