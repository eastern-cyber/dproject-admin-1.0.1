...
const sourceMap = sourceData.reduce((acc, entry) => {
  ...
  const userIdLower = userId.toLowerCase(); // Ensure case-insensitive comparison
  if (!acc[userIdLower] || parsedTimestamp < acc[userIdLower].parsedTimestamp) {
    acc[userIdLower] = { block_timestamp, parsedTimestamp };
  }
...
const updatedTarget = targetData.map((entry) => {
  const userIdLower = entry.userId.toLowerCase(); // Ensure case-insensitive comparison
  const planAValue = sourceMap[userIdLower]?.block_timestamp || "";
  if (!planAValue) {
